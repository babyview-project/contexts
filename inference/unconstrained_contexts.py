...
overall_video_dir = '/home/tsepuri/contexts/test_chunks/'
...
'''
def create_question():
    question = "This a video from the point-of-view of a camera mounted on a child's head. Is the child interacting with an object? Answer strictly in this format:" \
    "Object interaction:Yes/No\nObject category:<category> (if yes, otherwise 'None')"
    return question


def convergent_validity_question():
    question = "This a video from the point-of-view of a camera mounted on a child's head. What objects are actually present? Answer strictly in this format:" \
    "Object interaction:Yes/No\nObject category:<category> (if yes, otherwise 'None')"
    return question
'''
...
def get_model_responses_for_video_sublist(video_dir_sublist):
    prompter = VideoLLaMA(device="cuda")
    ...
    for dir_num, video_dir in enumerate(video_dir_sublist):
        ...
        out_df_path = os.path.join(out_vis_dir, os.path.basename(video_dir) + 'processed' + '.csv')
...
if __name__ == "__main__":
    ...
    print(video_chunks)
    #get_model_responses_for_video_sublist(video_chunks[0]) 
    # Run parallel tasks
