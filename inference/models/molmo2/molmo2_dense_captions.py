...
from models.molmo2.molmo2_constrained import Molmo2
from models.video.vqa import _set_seed
from constants import get_bv_main_ids
import argparse
...
python -m models.molmo2.molmo2_dense_captions \
    --video_dir /ccn2/dataset/babyview/unzip_2025_10s_videos_256p/ \
    --overwrite --output_path ./vis_model_predictions/2026_01_28/molmo2_freeform_throwaway \
    --num_processes 1
...
question = (
    "This video is recorded from the point-of-view of a child, with a camera mounted on the child's head. "
    "Answer all the questions. (1) What is the child wearing the camera doing? (2) Are there other people in the scene? If so, what are they doing? (3) Is anyone interacting with the child wearing the camera?"
)

@ray.remote(num_gpus=1)
def get_model_responses_for_video_sublist(video_dir_sublist, chunk_id, out_vis_dir, side_by_side_vis_dir, overwrite):
    prompter = Molmo2(use_constrained_decoding=False)
    ...
    for dir_num, video_dir in pbar_dirs:
        ...
        out_df = pd.DataFrame(columns=['video_id', 'dense_caption'])
        ...
        for vid_num, video_path in pbar_videos:
            ...
            try:
                ...
                while response is None:
                    ...
                    if not response:
                        print(f"Empty response on try {try_count}")
                        response = None
                ...
                row = {
                    ...
                    'superseded_gcp_name_feb25': re.sub(r"_processed*", "", video_path),
                    'video_path': video_path,
                    'dense_caption': response,
                }
...
if __name__ == "__main__":
    ...
    out_vis_dir = os.path.join(args.output_path, 'molmo2_dense_captions')
