...
python -m models.molmo2.molmo2_contexts \
    --video_dir /ccn2/dataset/babyview/unzip_2025_10s_videos_256p/ \
    --overwrite --output_path ./vis_model_predictions/2026_01_28/molmo2_throwaway \
    --num_processes 1
...
prompt_key_values = {
    ...
    # "Dense caption. Describe what the child wearing the camera is doing. Describe what other people are doing. Describe who is interacting with who": None,
}

def create_question():
    question = "This video is recorded from the point-of-view of a child, with a camera mounted on the child's head. Annotate this video by responding strictly only in this format with both keys and values: "
...
def get_model_responses_for_video_sublist(video_dir_sublist, chunk_id, out_vis_dir, side_by_side_vis_dir, overwrite):
    ...
    prompter = Molmo2(use_constrained_decoding=True)
        ...
        for vid_num, video_path in pbar_videos:
            ...
            try:
                ...
                while response is None or response_dict is None:
                    ...
                    print(f"Raw model response: {response[:200]}...")  # Debug: show first 200 chars
                    response_dict = prompter.convert_model_response_to_dict(response, list(prompt_key_values.keys()), prompt_key_values)
                    if response_dict is None:
                        print(f"Failed to parse response on try {try_count}")
...
if __name__ == "__main__":
    ...
    out_vis_dir = os.path.join(args.output_path, 'molmo2_constrained')
