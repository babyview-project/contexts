...
def split_video(video_path, chunk_size_sec, output_dir, keep_audio=True):
    """
...
def assign_top_label_and_probs(generations, image_paths, label_type, choices_regex,current_batch=None,choices_map=None):
    """
    ...
    for prompt_generation, image_path in zip(generations, image_paths):
        ...
        for i, output in enumerate(prompt_generation.outputs):
            ...
            if choices_map is not None and curr_text:
                curr_text = choices_map[curr_text]
            ...
            if i == 0:
                for logprob in output.logprobs[0].values():
                    if re.match(choices_regex, logprob.decoded_token):
                        curr_text = logprob.decoded_token
                        if choices_map is not None:
                            if curr_text in choices_map.keys():
                                curr_text = choices_map[curr_text]
                            else:
                                continue
                        text_options_dict[curr_text] = logprob.logprob
